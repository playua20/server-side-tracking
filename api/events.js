 ...
 *
 * `offset` is wrong for a table that is still being written to: a new event
 * arriving between two pages shifts every later row, so page 2 repeats a row
 * page 1 already showed, and something else is skipped entirely. It also gets
 * slower with depth, because the database has to walk and discard the rows it
 * skips. A cursor names the last row seen — "older than this exact event" —
 * which is both stable under inserts and a plain index seek.
 *
 * created_at alone is not unique (two events can share a millisecond), so the
 * cursor is the pair (created_at, id) and the comparison is lexicographic.
 */
...
function parseCursor(raw) {
  ...
  // Validated because both halves are interpolated into a PostgREST filter.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // The filter lists and the total only matter for the first page.
